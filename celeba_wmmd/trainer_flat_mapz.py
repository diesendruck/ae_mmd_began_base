...
parser.add_argument('--num_conv_filters', type=int,       default=16)
...
'''
g_enc, g_var = MapToEncoding(z,  reuse=False)
x_enc, d_var_enc = EncoderCNN( 
    x, channel, z_dim, repeat_num,
    num_conv_filters, data_format, reuse=False)
AE_x, d_var_dec = DecoderCNN(
    x_enc, num_conv_filters, channel,
    repeat_num, data_format, reuse=False)
g, _ = DecoderCNN(
    g_enc, num_conv_filters, channel,
    repeat_num, data_format, reuse=True)

g_enc_, _ = EncoderCNN( 
    g, channel, z_dim, repeat_num,
    num_conv_filters, data_format, reuse=True)
AE_g, _ = DecoderCNN(
    g_enc_, num_conv_filters, channel,
    repeat_num, data_format, reuse=True)
